...
description: "Create a new stream on EVM, Bitcoin, or Solana using the Moralis Admin Panel or the API to start monitoring blockchain addresses and receiving webhook data."
---
...
<Tabs>
  ...
  <Tab title="Bitcoin">
    ...
    <Note>
    Bitcoin Streams delivers each matched transaction **twice** — once when it enters the mempool (`confirmed: false`) and again when the block is mined (`confirmed: true`). Dedupe on `txid` and upsert on the second delivery to flip your local confirmation flag.
    </Note>
  </Tab>
  <Tab title="Solana">
    ## Option 1: Create via Admin Panel
