...
<Tabs>
  ...
  <Tab title="Bitcoin">
    Bitcoin finality is **proof-of-work based**. Once a transaction is included in a mined block, each subsequent block adds another confirmation. Streams sends:

    - `confirmed: false` when the transaction enters the mempool
    - `confirmed: true` once the configured confirmation depth is reached

    Bitcoin reorgs follow the longest-valid-chain rule and become exponentially less likely with each additional confirmation. See [Supported Chains](/streams/supported-chains) for the threshold Streams uses.
  </Tab>
  <Tab title="Solana">
