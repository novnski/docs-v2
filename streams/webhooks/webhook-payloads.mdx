...
description: "Understand the structure of Streams webhook payloads across EVM, Bitcoin, and Solana — including transactions, transfers, logs, confirmations, retries, and signature verification."
---
...
<Tabs>
  ...
  <Tab title="Bitcoin">
    ...
    | Field       | Description                                                                |
    | :---------- | :------------------------------------------------------------------------- |
    | `confirmed` | `false` for the mempool delivery, `true` once the block is mined           |
    | `chainId`   | `btc-mainnet`                                                              |
    | `streamId`  | Unique stream identifier                                                   |
    | `tag`       | Optional stream tag                                                        |
    | `retries`   | Number of delivery attempts                                                |
    ...
    <Warning>
    Even with `includeInputs: true`, input `address` and `value` fields return `null`. Bitcoin Streams reliably detects **inbound** transfers to watched addresses, not outflows. See [Bitcoin Streams](/streams/bitcoin-streams) for the full list of behaviors.
    </Warning>
  </Tab>
  <Tab title="Solana">
    ```javascript
...
1. **Unconfirmed (`confirmed: false`)** — sent as soon as the block is mined / slot is produced (or when the transaction enters the mempool, on Bitcoin).
2. **Confirmed (`confirmed: true`)** — sent once finality is reached for the chain.
...
<Tabs>
  ...
  <Tab title="Bitcoin">
    ...
    Each Bitcoin payload begins with the block the matched transaction belongs to (or the mempool sentinel for unconfirmed deliveries):
    ...
    1. **Mempool** — `confirmed: false`, sent the moment Moralis sees the transaction enter the mempool.
    2. **Confirmed** — `confirmed: true`, sent when the transaction is included in a mined block.
