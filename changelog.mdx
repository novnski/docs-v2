...
<Update label="July 31, 2026" tags={["Deprecation"]}>
  ...
  **Solana token holders:**
  ...
  - `GET /token/{network}/holders/{address}` — migrate to [Token Holders](/data-feeds/recipes/token/token-holders)
  - `GET /token/{network}/{address}/top-holders` — migrate to [Token Holders](/data-feeds/recipes/token/token-holders)
  - `GET /token/{network}/holders/{address}/historical` — migrate to [Historical Balances](/data-feeds/recipes/token/historical-balances)

  **EVM token holders (historical only):**

  - `GET /erc20/{tokenAddress}/holders/historical` — migrate to [Historical Balances](/data-feeds/recipes/token/historical-balances)
  ...
  **Solana token discovery (`/token/{network}/exchange/*`):**
  ...
  All four are served by [Token Bonding Status](/data-feeds/recipes/token/token-bonding-status).
