...
<Update label="August 26, 2026" tags={["Deprecation"]}>
  ## Moonbeam, Moonriver and Lisk Support Ends September 25, 2026

  Moralis is removing support for **Moonbeam** (`0x504`), **Moonriver** (`0x505`), and **Lisk** (`0x46f`) on **September 25, 2026**, across all products: Data API, Streams, RPC Nodes, and Datashare.

  GLMR and MOVR have already migrated to Base ([Moonbeam](https://moonbeam.network/news/moonbeam-strategic-update-moonbeam-network-relaunches-on-base/), [Moonriver](https://moonbeam.network/news/moonriver-strategic-update-movr-is-migrating-to-base/)), and both original networks stopped producing new blocks on August 1, 2026. Lisk is winding down the Lisk Chain as part of [the next chapter for Lisk](https://lisk.com/blog/posts/introducing-the-new-lisk/). In both cases the activity has moved to chains Moralis already supports in full.
  ...
  **Timeline**
  ...
  | September 25, 2026 | Moralis removes all three chains. Requests targeting them return an unsupported-chain error |
  | October 31, 2026 | Lisk's deadline to bridge assets from the Lisk Chain to Ethereum. Bridging takes at least 7 days |
  | November 30, 2026 | Final shutdown of the Lisk Chain and its related infrastructure |

  **Where to point your integration**

  | Removed chain | Migrate to |
  | --- | --- |
  | Moonbeam (`moonbeam` / `0x504`) | Base (`base` / `0x2105`) |
  | Moonriver (`moonriver` / `0x505`) | Base (`base` / `0x2105`) |
  | Lisk (`lisk` / `0x46f`) | Ethereum (`eth` / `0x1`) or Base (`base` / `0x2105`) |

  For RPC Nodes, swap the path segment, for example `https://site1.moralis-nodes.com/base/`. Your API key, authentication, request shapes, and response formats do not change. Only the target chain does.

  GLMR and MOVR use new contracts on Base. LSK is unchanged on Ethereum, so if you hold it there or on an exchange, no action is needed.
  ...
  **What changes on September 25, 2026**
  ...
  - **Data API:** wallet, token, NFT, DeFi, and price endpoints reject `moonbeam`, `0x504`, `moonriver`, `0x505`, `lisk`, and `0x46f`. This applies to the `chain` query parameter on Deep Index endpoints and to the `chains` array on Universal API endpoints, so remove these values from your chain lists rather than relying on the rest of a multi-chain request still resolving.
  - **RPC Nodes:** the `moonbeam`, `moonriver`, and `lisk` paths on `site1.moralis-nodes.com` and `site2.moralis-nodes.com` stop resolving, including archive reads at historical block heights (`eth_getBlockByNumber`, `eth_getLogs`, `eth_getBalance` with a past block tag).
  - **Streams:** any stream that still lists `0x504`, `0x505`, or `0x46f` in `chainIds` is invalid. List your streams with [Get streams](/streams/api-reference/streams/get-streams), then either [update the stream](/streams/api-reference/streams/update-stream) to a supported chain or [delete it](/streams/api-reference/streams/delete-stream).
  - **Datashare:** all three chains are removed from the available datasets.
  - **Docs:** the Moonbeam and Lisk RPC reference pages and their rows in the supported-chains tables are removed from the navigation.
  ...
  **Export your history before September 25, 2026**
  ...
  Historical data stays queryable until then. Paginate each call with `cursor` until the response returns an empty `result` and no `cursor`.
  ...
  **The Lisk Chain runs past our removal date**

  Moralis removes Lisk ahead of Lisk's own timeline. The Lisk Chain keeps producing blocks and its infrastructure keeps running as normal until November 30, 2026, so if you need chain access after September 25, 2026, point your integration at Lisk's own endpoints. Lisk's [documentation](https://docs.lisk.com/) stays available and user-facing support has moved to [support.lisk.com](https://support.lisk.com/).

  For a walkthrough of the Moonbeam and Moonriver migration, watch [Moonbeam and Moonriver Migration to Base](https://www.youtube.com/watch?v=63UJ4EaqfUM).
...
<Update label="August 11, 2026" tags={["Deprecation"]}>
  ...
  <Note>
    **Updated August 26, 2026.** Both chains stopped producing new blocks on August 1, 2026, so there has been no real-time data to serve since then. Historical data stayed available and is now scheduled for removal on **September 25, 2026**. See [Moonbeam, Moonriver and Lisk Support Ends September 25, 2026](#moonbeam-moonriver-and-lisk-support-ends-september-25-2026) for the current timeline.
  </Note>
...
<Update label="July 31, 2026" tags={["Deprecation"]}>
  ## Token Holders and Solana Token Discovery Endpoints Removed

  As [announced on June 30](#token-holders-and-solana-token-discovery-endpoints-sunsetting), the following endpoints have now been **removed** as of July 31, 2026. Requests to these endpoints return a 404. If you are still calling any of them, migrate to the replacements below.

  **Solana token holders:**

  - `GET /token/{network}/holders/{address}` — migrate to [Token Holders](/data-feeds/templates/token/token-holders)
  - `GET /token/{network}/{address}/top-holders` — migrate to [Token Holders](/data-feeds/templates/token/token-holders)
  - `GET /token/{network}/holders/{address}/historical` — migrate to [Historical Balances](/data-feeds/templates/token/historical-balances)

  **EVM token holders (historical only):**

  - `GET /erc20/{tokenAddress}/holders/historical` — migrate to [Historical Balances](/data-feeds/templates/token/historical-balances)
  ...
  **Solana token discovery (`/token/{network}/exchange/*`):**
  ...
  All four are served by [Token Bonding Status](/data-feeds/templates/token/token-bonding-status).

  The API reference pages for these endpoints have been removed from the navigation, and their entries are gone from the OpenAPI sources so generated docs and API clients no longer advertise them.

  If you need help identifying the right replacement, [reach out to the team](https://admin.moralis.com/) (via chat widget).
</Update>
...
<Update label="July 20, 2026" tags={["New"]}>
  ...
  **What's included:**
  ...
  - [Data Feed Templates](/data-feeds/templates/overview): 26 pre-built blueprints that reconstruct our most popular endpoints across Wallet, Token, Price, DeFi, NFT, and Blockchain APIs.
  - [Migration guides](/data-feeds/migration/overview): field-by-field mappings from the Moralis REST API to templates, including an [endpoint lookup](/data-feeds/migration/endpoints).
...
<Update label="June 30, 2026" tags={["Deprecation"]}>
  ...
  <Note>
    **These endpoints were removed on July 31, 2026.** See [Token Holders and Solana Token Discovery Endpoints Removed](#token-holders-and-solana-token-discovery-endpoints-removed) for the replacements.
  </Note>
  ...
  **Solana token holders (all sunset):**
  ...
  - `GET /token/{network}/holders/{address}` — Token holder metrics
  - `GET /token/{network}/{address}/top-holders` — Top token holders
  - `GET /token/{network}/holders/{address}/historical` — Historical token holders
  ...
  **Solana token discovery (`/token/{network}/exchange/*`):**
  ...
  - `GET /token/{network}/exchange/{exchange}/new` — New tokens by exchange
  - `GET /token/{network}/exchange/{exchange}/bonding` — Bonding tokens by exchange
  - `GET /token/{network}/exchange/{exchange}/graduated` — Graduated tokens by exchange
  - `GET /token/{network}/{address}/bonding-status` — Token bonding status
</Update>
