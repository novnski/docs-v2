...
- **Multichain queries:** fetch positions from a single chain, multiple chains, or all EVM chains in one API call via the universal endpoint at `api.moralis.com`
...
- `getDefiSummary` — every protocol a wallet holds positions in, total USD value, and unclaimed rewards
- `getDefiPositionsSummary` — all positions with token types, values, and reward tokens
- `getDefiPositionsByProtocol` — detailed per-protocol breakdown with position-level detail
