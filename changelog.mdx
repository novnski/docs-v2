...
<Update label="April 29, 2026" tags={["New Feature"]}>
  ## Streams: Now on Solana

Moralis Streams now supports **Solana mainnet**. The same product, the same delivery model (webhook push to your endpoint) - extended to Solana.

If you're already using Streams on EVM, the developer experience is familiar: define a stream, set filters, receive webhooks. The differences are in **what you filter on**, because Solana's transaction model is different from EVM's.

**Filters available at launch:**

- `addresses` — match transactions where any `accountKey` in the transaction matches one of the watched addresses
...
- `allAddresses` — receive every transaction on the network

**What's in the payload:** transaction `signature`, full `accountKeys`, top-level and inner instructions, and pre / post SPL token balances - everything you need to compute deltas without replaying instruction logic.

**Usecases:** wallet activity notifications, DEX / DeFi position monitoring, SPL token transfer tracking (airdrops, payments, compliance), bot monitoring and MEV detection, and AI agents reacting to on-chain events.
...
<Update label="April 29, 2026" tags={["New Feature"]}>
  ## Cross-Chain Token Balances on the Universal API

One of our most popular Deep Index endpoints, **Token Balances**, is now available on the Universal API as **Cross-Chain Token Balances**.

The headline change: **a single request returns a wallet's token balances across multiple chains.** Previously, getting a multi-chain portfolio meant one request per chain and stitching results together client-side. Now it's one call.

**What changed vs. the legacy endpoint:**

|                | Legacy (`getWalletTokenBalancesPrice`) | New (`getTokenBalances`)              |
| -------------- | -------------------------------------- | ------------------------------------- |
...
Everything you already rely on - spam filtering, unverified contract exclusion, liquidity thresholds, USD pricing - works exactly the same. The only difference is you can now ask for multiple chains in a single call.

**Usecases:** wallets, exchanges, portfolio dashboards, tax tools, and AI agents that need a wallet's full multi-chain holdings without firing one request per chain.

**Roadmap:** Bitcoin support comes next, followed by Solana. The end state is one endpoint, one request shape, balances across EVM, Bitcoin, and Solana wallets.

[Explore Cross-Chain Token Balances →](/data-api/universal/wallet/cross-chain-token-balances)
