...
<Update label="August 11, 2026" tags={["Deprecation"]}>
  ...
  Moralis API support for **Moonbeam** (`0x504`) and **Moonriver** (`0x505`) will be removed on **August 21, 2026**.
  ...
  **What's changing:**
  ...
  - **Data API:** wallet, token, NFT, DeFi, and price endpoints no longer return Moonbeam or Moonriver data. Requests passing `moonbeam` / `0x504` or `moonriver` / `0x505` will return an error.
  - **RPC Nodes:** the Moonbeam and Moonriver RPC endpoints (`https://site1.moralis-nodes.com/moonbeam/` and `.../moonriver/`) are no longer available, and their documentation will be removed.
  - **Streams:** existing Moonbeam and Moonriver streams will stop emitting webhook notifications and should be deleted from your workspace.
  ...
  **Not affected:** your API key, authentication, request shapes, and response formats. Base is already fully supported across the Data API, RPC Nodes, and Streams. Only the target chain changes.
  ...
  Make these changes before August 21 to avoid errored responses. Questions? [Reach out to the team](https://admin.moralis.com/) (via chat widget).
</Update>
