...
Let's make your first request using the [Wallet API](/data-api/evm-api/wallet-api/index).\
In this example, we'll retrieve **token balances** for a wallet on Ethereum - including **formatted balances, USD values, prices, and metadata** - with a single API call using [Token Balances](/data-api/evm-api/wallet-api/token-balances).
