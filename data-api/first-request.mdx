...
Let's make your first request using the [Wallet API](/data-api/wallet-api/index).\
In this example, we'll retrieve **token balances** for a wallet on Ethereum - including **formatted balances, USD values, prices, and metadata** - with a single API call using [Token Balances](/data-api/wallet-api/endpoints/token-balances).
...
<Info>
  Replace `YOUR_API_KEY` with the API key from your Moralis dashboard.
</Info>
