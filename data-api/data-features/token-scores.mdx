...
In addition to the latest score, Moralis provides [historical Token Scores](/universal-api/token/token-score-timeseries), allowing you to track how a token’s onchain quality evolves over time.
...
Developers can fetch the historical scoring breakdown using the [Timeseries Token Score](/universal-api/token/token-score-timeseries) endpoint:
