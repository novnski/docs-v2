---
title: "Token Scores"
description: "The Moralis Token Score is a unified 0-100 rating that evaluates the onchain quality of a token using six weighted metrics, supported across all EVM chains and Solana mainnet. "
tag: "New"
---
...
In addition to the latest score, Moralis provides [historical Token Scores](/data-api/universal-api/token-api/token-score-timeseries), allowing you to track how a token’s onchain quality evolves over time.
...
Developers can fetch the historical scoring breakdown using the [Timeseries Token Score](/data-api/universal-api/token-api/token-score-timeseries) endpoint:
...
Developers can fetch the full scoring breakdown using the [Token Score](/universal-api/token-api/endpoints/token-score) endpoint:
