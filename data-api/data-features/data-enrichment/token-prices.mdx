---
title: "Token Prices"
description: "Learn how Moralis token prices are derived from onchain DEX activity, including pool selection, pair-based pricing, liquidity filtering, inactivity rules, and mainnet-only behavior."
---
...
This makes token prices safe to use in downstream features such as [Token Balances](/data-api/wallet-api/endpoints/token-balances), [Wallet Net Worth](/data-api/evm-api/wallet-api/net-worth), [PnL](/data-api/evm-api/wallet-api/pn-l/wallet-pn-l) and many more.
...
- [Token price by token address](/evm-api/price-api/endpoints/token-price)
- [Token price by pair address](/data-api/evm-api/token-api/swaps-and-pairs/pair-stats)
...
To avoid unreliable prices, Moralis enforces minimum liquidity requirements.

Default thresholds:

- **EVM chains:** \$50 minimum pair-side liquidity
- **Solana:** no enforced liquidity threshold

“Pair-side liquidity” refers to the liquidity of **the token being priced**, not the counter asset.

Liquidity filtering applies to both token-based and pair-based price queries.
