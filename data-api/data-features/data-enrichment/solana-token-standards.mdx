...
description: "Understand the Solana token standards supported by Moralis, including the SPL Token Program, Token-2022, and Metaplex metadata standard."
---
...
**Token-2022** is the next-generation token program on Solana, introducing built-in extensions such as:

- **Transfer fees** — Enforce fees on every token transfer
- **Confidential transfers** — Hide transfer amounts using zero-knowledge proofs
- **Transfer hooks** — Execute custom logic on each transfer
...
Moralis supports Token-2022 tokens across its Solana API endpoints.
