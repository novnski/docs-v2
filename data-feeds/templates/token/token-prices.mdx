...
The template lands **one row per in-block price update**: Moralis' continuous mark for each `(token, pair, protocol)` at the block it changed. A token traded across several pairs or protocols produces several marks per block; the table keeps them all, keyed for by-token lookups.
...
ClickHouse uses the collapsing log-table pattern (see the [templates overview](/data-feeds/templates/overview#destinations)) so chain reorganizations self-correct. The fact table's sort key is token-first, so a token's full price history is a contiguous range read; the `latest_token_price_dict` dictionary holds the carry-forward mark for quiet positions.

### Full schema

Below is the complete read table this template produces. It's a starting point: keep the columns you need and drop the rest (see [Schema & flexibility](/data-feeds/templates/overview#schema--flexibility)). `usd_price` and `native_price` are carried as text to preserve full precision across many orders of magnitude; cast at read time.
...
-- By-token price history (the template's primary purpose).
...
`tokenPriceUpdates` is one of the smaller slices in the feed. A continuous real-time feed measured:

| Chain | Typical month (P50) | Busy month (P90) |
| --- | --- | --- |
| Solana | **~84 GB** | **~110 GB** |
| BNB Smart Chain | **~23 GB** | not measured |
| Base | **~6.0 GB** | not measured |
| Polygon | **~3.0 GB** | not measured |
| Arbitrum | **~2.7 GB** | not measured |
| Ethereum | **~1.3 GB** | not measured |
| Optimism | **~0.9 GB** | not measured |

<sub>Measured 2026-07-30, `metered` delivery; Solana over 25 sampled partitions, the EVM chains over 60. Billing is on uncompressed bytes. All runs included the negligible `deployedContracts` slice.</sub>

**Size your allowance against a busy month, not a typical one.** Where only a typical figure exists, leave meaningful headroom above it rather than sizing to it — chain activity is bursty, and busy months on comparable slices have measured 30–70% above typical. Three caveats:
...
- **Measured chains only.** Volume concentrates hard per chain, so these figures do not extrapolate to unmeasured chains.
- **Filtering does not reduce it.** Restricting to specific tokens or venues happens after the data is delivered. To reduce cost, narrow the **block range**, not the entity set.
...
This template replaces both REST price endpoints. Their responses were **not 1:1 rows in one table**: prices, token details, and pair liquidity came bundled in a single payload, but they live in different parts of the dataset here.

<Note>
  Rebuilding the full REST response takes a **combination of templates**: this one for the marks, plus [Token Metadata](/data-feeds/templates/token/token-metadata) for `tokenName` / `tokenSymbol` / `tokenDecimals` and [Pair Reserves](/data-feeds/templates/markets/pair-reserves) if you need `pairTotalLiquidityUsd` (reserves × price). Join them at read time on `token_address` / `pair_address`.
</Note>
...
| `nativePrice` | latest `native_price` (a scalar, not the REST endpoint's object) | calculated |
...
| `tokenName`, `tokenSymbol`, `tokenDecimals` | [Token Metadata](/data-feeds/templates/token/token-metadata) template | exact |
| `24hrPercentChange`, `usdPrice24hr*` | derive: mark ~24h ago vs latest, from this table's history | calculated |
| `pairTotalLiquidityUsd` | [Pair Reserves](/data-feeds/templates/markets/pair-reserves) template (reserves × price) | calculated |
| `exchangeAddress` | not in these templates: `protocol` names the venue; bring a factory list if you need the address | add yourself |
...
Replaced by the newest mark for one token in this template's table, joined to [Token Metadata](/data-feeds/templates/token/token-metadata) for the token details the endpoint bundled in. On Postgres:
