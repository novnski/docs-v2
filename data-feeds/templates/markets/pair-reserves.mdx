---
title: "Pair Reserves"
description: "Sync the current reserve0 / reserve1 of any DEX pair, the pool's absolute token balances, kept latest-wins per pair, into your own database. Mirrors the Moralis GET /{pair_address}/reserves endpoint."
---
...
Mirrors Moralis `GET /{pair_address}/reserves` → `{ reserve0, reserve1 }`. The template lands the pool's absolute token balances and keeps the **latest observation per pair**, so a single read returns each pair's current reserves.

### What you get

The template lands **one observation per swap**, the pool's absolute token balances captured *after* each trade, and the current reserves of a pair are the highest-`(block_number, log_index)` observation for that pair:
