...
description: "Using Streams webhooks today? Here is how the model maps onto Data Feeds — push webhooks vs. data flowing into a database you own — and what to expect from the upcoming per-stream migration guides."
---
...
<Note>
**Individual Streams migration guides are coming soon.** This page gives you the model so you can evaluate the
move today. When the per-stream guides land, each will map a common stream configuration to its matching
recipe, field by field — the same treatment the [Data API migration guides](/data-feeds/migration/overview)
give REST endpoints.
</Note>
...
You do not need to wait for the per-stream guides to evaluate Data Feeds:
