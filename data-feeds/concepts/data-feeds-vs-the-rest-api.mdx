---
title: "Data Feeds vs. the REST API"
description: "How results from Moralis Data Feeds compare to a Web3 API endpoint, what comes through exactly, what you calculate, and what you add yourself."
---

A Moralis Web3 API endpoint hands you a single, **pre-packaged, enriched response**. Data Feeds give you
...
> The REST endpoint is a *finished dish*. A Data Feed is the *ingredients*, the same onchain facts, plus
> live prices, ready for you to plate however you like (or let a template plate them for you).
...
- **Prices come from real onchain trades.** Data Feeds derive USD values from DEX activity in the data lake, so
  they're live and verifiable, and may differ slightly from the REST API's pricing engine. A token that hasn't
...
- **You own the filtering.** The REST endpoint bakes in choices (which tokens to include, spam rules); with a
