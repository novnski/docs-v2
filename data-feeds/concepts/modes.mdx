...
description: "A template runs in one of three modes. The mode decides whether the sink backfills past history, tails live blocks, or does both. Here's how to pick."
---

# Modes

Every template runs in one of three modes. The mode is a single setting, but it decides the shape of your
...
straight into realtime tailing. This is the default for most templates because it gives you a complete table
...
The mode is set in the template's sink config, alongside the start block:
...
and MySQL template variants ship in `historical` mode for backfill loads. See [Destinations](./destinations)
