...
This recipe replaces the collection-metadata side of the legacy NFT endpoints. Data Feeds are **not** 1:1 replicas of those responses — you land the underlying data in your own database and reconstruct the response shape with a query. The subsections below cover each legacy endpoint: what replaces it, the field mapping (**exact** = straight from the chain, **calculated** = derived from real trades, very close, **add yourself** = off-chain signal not in the feed), and the endpoint-specific gotchas.
...
- `contract_type` may be the `NFT` fallback when a contract advertises neither EIP-165 interface; the legacy endpoint made a similar best-effort call.
