...
<Warning>
Reproducing the full REST response needs **three recipes**, not one. The transfer event carries no collection metadata or sale context. Run this recipe plus [NFT Collection Metadata](/data-feeds/recipes/nft/nft-collection-metadata) (`token_name` / `token_symbol`) and [NFT Trades](/data-feeds/recipes/nft/nft-trades) (`last_sale`) against the same database and join them.
</Warning>
