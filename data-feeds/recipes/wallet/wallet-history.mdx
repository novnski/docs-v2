...
Coming from the legacy Web3 Data API? This recipe replaces the wallet-activity endpoints below. Data Feeds are **not 1:1 replicas** of REST responses (you own the table and reshape it with SQL), so each subsection states exactly what maps, what's calculated for you, and what you add yourself. For the general model see [Data Feeds vs. the legacy API](/data-feeds/concepts/data-feeds-vs-the-legacy-api); for the full endpoint index see the [endpoint map](/data-feeds/migration/endpoints).
