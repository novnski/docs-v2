...
- The **recipe bundle** you receive with access (includes a `docker-compose` for a local database plus the sink).

**Start the sink**

```bash
cd recipes
cp .env.example .env      # set MORALIS_API_KEY (cm_live), MORALIS_CHAIN_ID, MORALIS_HISTORICAL_FROM_BLOCK
docker compose -f docker-compose.recipes.yml up -d
./run-recipe.sh walletHistoryFull clickhouse   # or: postgres | mysql
```

It backfills the window you choose, then (on ClickHouse) tails live blocks. Set the window in `.env`:

```bash
MORALIS_HISTORICAL_FROM_BLOCK=tip-1000   # last ~1000 blocks, a fast first run
# tip-43200 ≈ 1 day on a 2s chain, or an absolute block number for a fixed window
```
