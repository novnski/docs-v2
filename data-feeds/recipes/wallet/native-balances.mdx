...
This recipe replaces the endpoint on its own: the latest observation per wallet **is** the response's `balance`, exact and straight from the chain — no other recipe required. `GET /wallets/balances` is the same read across many wallets.
