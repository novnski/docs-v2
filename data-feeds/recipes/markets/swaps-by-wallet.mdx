...
This recipe replaces the wallet-swaps REST endpoint. It is **not** a 1:1 replica of the response, you land the trades in your own database and reconstruct the response shape with a query. Fidelity labels: **exact** = straight from the chain, **calculated** = derived from real DEX trades (very close, compare with a tolerance), **add yourself** = off-chain signal not in the feed.

### GET /wallets/:address/swaps

A wallet's DEX trade history with bought/sold legs and USD values. Replaced directly by this recipe, trades are keyed by `trader_address`, so the REST response is a single by-wallet read, with [Token Metadata](/data-feeds/recipes/token/token-metadata) joined in for the leg symbols and `pairLabel`.
