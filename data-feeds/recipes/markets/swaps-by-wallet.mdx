...
A wallet's DEX trade history with bought/sold legs and USD values. Replaced directly by this recipe, trades are keyed by `trader_address`, so the legacy response is a single by-wallet read, with [Token Metadata](/data-feeds/recipes/token/token-metadata) joined in for the leg symbols and `pairLabel`.
...
WHERE s.trader_address = lower($1)
  AND ($2::bigint IS NULL OR s.block_number < $2)   -- keyset pagination (legacy cursor)
