...
description: "Sync every ERC-20 token transfer, by token or by wallet, as a flat, ordered event log into your own database."
---
...
This recipe replaces both legacy ERC-20 transfer endpoints from one sync: the same `token_transfers` table serves the by-token and by-wallet reads.

<Note>
  **Not 1:1.** The legacy responses inlined `token_name` / `token_symbol` / `token_decimals` and a pre-scaled `value_decimal`; this recipe's `amount` is raw `uint256`. Pair it with [Token Metadata](/data-feeds/recipes/token/token-metadata) and join on `token_address` to scale amounts and label tokens. `token_logo`, `possible_spam`, `verified_contract`, `security_score`, and the address `*_label` / `*_entity` fields are off-chain signals; add them from your own token/label lists or drop them.
</Note>
...
| `token_name`, `token_symbol`, `token_decimals` | join [Token Metadata](/data-feeds/recipes/token/token-metadata) | exact |
| `token_logo`, `possible_spam`, `verified_contract`, `security_score` | — | add yourself |
| `from/to_address_label`, `*_entity`, `*_entity_logo` | — | add yourself |
