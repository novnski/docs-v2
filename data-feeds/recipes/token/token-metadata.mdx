...
| `fully_diluted_valuation` | `total_supply_formatted × usd_price` via [Token Prices](/data-feeds/recipes/token/token-prices) | calculated |
...
- The recipe is **EVM-shaped**: SPL token metadata doesn't flow through `deployedContracts` (see [Multichain](#multichain)).
