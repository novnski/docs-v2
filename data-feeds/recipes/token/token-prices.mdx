...
<Note>
  Rebuilding the full REST response takes a **combination of recipes**: this one for the marks, plus [Token Metadata](/data-feeds/recipes/token/token-metadata) for `tokenName` / `tokenSymbol` / `tokenDecimals` and [Pair Reserves](/data-feeds/recipes/markets/pair-reserves) if you need `pairTotalLiquidityUsd` (reserves × price). Join them at read time on `token_address` / `pair_address`.
</Note>
