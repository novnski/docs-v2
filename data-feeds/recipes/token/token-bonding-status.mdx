...
Lands one row per **launchpad event** (`created` / `traded` / `migrated`) from the decoded `launchpadEvents` feed, and derives a per-token latest-status projection. The four deprecated pump.fun surfaces, new, bonding, graduated, and per-token status, are all reads over this one recipe.
...
**Size your allowance against the top of the band, not the middle.** Chain activity varies, and this recipe tracks launchpad activity specifically, which is bursty by nature: a quiet month and a memecoin frenzy are not the same month. Worked example, against a hypothetical 10 GB monthly allowance:
...
Both are inside the measured range for this recipe, so choose an allowance that covers the busy figure and treat the typical figure as the good case.
...
- **Solana.** On EVM chains the `launchpadEvents` array is effectively unpopulated (see [Multichain](#multichain)), so this recipe reads close to nothing on Ethereum.
...
<Warning>
All four Solana launchpad endpoints below are **deprecated and sunset on July 31, 2026**. This one recipe replaces all of them, each REST response is a read over the tables above.
</Warning>
