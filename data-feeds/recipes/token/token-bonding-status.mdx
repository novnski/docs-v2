...
<RecipeMigrationCallout
  items={[
    { path: "GET /token/{network}/exchange/{exchange}/new", guide: "solana-new-tokens-by-exchange", label: "New tokens by exchange" },
    { path: "GET /token/{network}/exchange/{exchange}/bonding", guide: "solana-bonding-tokens-by-exchange", label: "Bonding tokens by exchange" },
    { path: "GET /token/{network}/exchange/{exchange}/graduated", guide: "solana-graduated-tokens-by-exchange", label: "Graduated tokens by exchange" },
    { path: "GET /token/{network}/{address}/bonding-status", guide: "solana-token-bonding-status", label: "Token bonding status" },
  ]}
/>
...
| `pool_address` | The DEX pool a token migrated into (on `migrated`) — *not populated on the Solana feed today* |
...
**Graduated tokens** — tokens with a `migrated` event (`pool_address` is empty on the Solana feed today, so drop it or resolve the pool from your own source):

```sql
SELECT token_address,
       minIf(event_ts, event_type = 'migrated') AS graduated_at,
       anyLast(pool_address)                    AS pool_address
FROM recipe_token_bonding_status.fact_launchpad_events FINAL
WHERE exchange_address = '6EF8rrecthR5Dkzon8Nwu78hRvfCKubJ14M5uBEwF6P'
GROUP BY token_address
...
- **No price in the feed.** The Solana `launchpadEvents` carry no price — the `current_price` and `pool_address` columns exist but land **empty on Solana today**. Derive the **native price** per token from `traded` events (`native_amount / token_amount`), and convert to **USD** with a SOL/USD mark (e.g. wrapped SOL from a [Token Prices](/data-feeds/recipes/token/token-prices) sync). **Liquidity** (cumulative curve `native_amount`, or post-graduation [Pair Reserves](/data-feeds/recipes/markets/pair-reserves)) and **FDV** (`token_metadata.total_supply` × price) are derive-or-omit.
- **`logo`** and other off-chain metadata aren't in the feed.

Core fields — `token_address`, `exchange_address`, `event_type`, `progress_percentage`, graduation (the `migrated` event), token name/symbol (on `created`), trade amounts, and timestamps — are sourced from `launchpadEvents`.
