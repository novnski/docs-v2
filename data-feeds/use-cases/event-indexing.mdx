...
description: "A Data Feeds recipe bundle for indexing raw event logs (every event from a contract, or one event signature across all contracts) into your own database for custom decoding and analytics."
---
...
| **Raw log recipes** (this use case) | A custom/unsupported contract or event, or you want *every* event a contract emits, or one signature chain-wide. You'll decode `data` against your own ABI. |
