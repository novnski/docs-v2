---
title: "Onchain Event Indexing"
description: "A Data Feeds template bundle for indexing raw event logs (every event from a contract, or one event signature across all contracts) into your own database for custom decoding and analytics."
---
...
### Raw logs vs decoded templates

Data Feeds ships two kinds of event data. Reach for raw logs when the decoded path doesn't cover what you need:

| Use… | When |
| --- | --- |
| **Decoded templates** (Token Transfers, Token Approvals, Swaps, NFT Transfers) | The event is a standard, already-decoded type: you want typed columns, no ABI decoding. |
| **Raw log templates** (this use case) | A custom/unsupported contract or event, or you want *every* event a contract emits, or one signature chain-wide. You'll decode `data` against your own ABI. |

### The template bundle

| Template | Role |
| --- | --- |
| [Contract Logs](/data-feeds/templates/logs/contract-logs) | Every raw event log emitted by a specific contract, in on-chain order. The "index my protocol's contracts" path. Optional `(contract, topic0)` grouping for high-volume contracts. |
| [Logs by Event Signature](/data-feeds/templates/logs/logs-by-topic0) | One event signature (`topic0`) across **all** contracts, chain-wide: every `Transfer`, `Approval`, `Swap`, etc. The "index one event everywhere" path. Optional `(topic0, contract)` grouping. |
...
Both templates are intentionally raw: `topic0` is the event signature hash, `topic1…topic3` are the indexed parameters, and `data` is the ABI-encoded non-indexed payload as hex. Decode `data` (and interpret the indexed topics) against the event ABI in your application. Data Feeds delivers the complete, ordered, reorg-safe primitives; the decode layer stays under your control.

### Notes and considerations

- **EVM only.** Raw log extraction covers EVM event logs (`topic0…3` + `data`). Solana program logs are a different model.
- **Heavier source.** Logs come from the wide raw `block` data (all logs and transactions per block), so per-block volume is higher than the compact decoded templates. `tx_hash` is recovered by joining each log to its sibling transaction row; drop that join if you don't need it.
...
- **Pair with decoded templates.** Use raw logs for the long tail and custom events; use the decoded templates where a typed, ready-made shape already exists.
