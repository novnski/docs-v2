---
title: "Trading & Charting"
description: "A Data Feeds template bundle for trading terminals, charting UIs, and bots: every DEX trade per pool and per token, candles, live reserves, and USD marks, synced into your own database."
---
...
This use case combines the markets templates. **Swaps by Pair** is the trade backbone (one row per fill); **Pair OHLCV** is a downstream aggregation of that same trade ingest; **Swaps by Token** re-keys the trades for token-centric views; **Pair Reserves** and **Token Prices** add live state and valuation.

| Template | Role in the terminal |
| --- | --- |
| [Swaps by Pair](/data-feeds/templates/markets/swaps-by-pair) | The trade backbone: every fill on a pool/pair (direct pool fills and aggregator-routed trades), one row per trade, USD-enriched in-block. The "trade tape" for a pool. |
| [Swaps by Token](/data-feeds/templates/markets/swaps-by-token) | The same trades re-keyed by token: every trade that touched token X, found under either side of the pair. The token-centric tape. |
| [Pair OHLCV](/data-feeds/templates/markets/pair-ohlcv) | Time-bucketed open/high/low/close + volume + trade count per pool, built by aggregating the swap ingest into candles. |
| [Pair Reserves](/data-feeds/templates/markets/pair-reserves) | Current `reserve0` / `reserve1` per pool, read from each swap's pool post-balances: live depth without a running sum. |
| [Token Prices](/data-feeds/templates/token/token-prices) | USD/native mark history per token, plus a latest-mark dictionary for carry-forward valuation of quiet positions. |
...
- **The swap templates** all draw from the same normalized in-block trade events, direct pool fills (`tokenSwaps`) and aggregator-routed trades (`aggregateTokenSwaps`), so every fill lands exactly once. Key by pool for a pool's tape, by token for a token's tape.
...
A trading terminal's pool screen wants the live tape, the candles, the current depth, and a mark: each from its own template, keyed by the same pool. Read ClickHouse with `FINAL` or a sign-aware aggregate so reorg `±1` pairs collapse before you aggregate; never a bare `WHERE sign = 1`.
...
- **EVM + Solana.** The trade, candle, reserve, and price templates are chain-parametrized. On Solana, repeated `logIndex` within one instruction means the pool-fill `vendor_event_id` should be widened (e.g. with the token addresses/amounts) to keep rows distinct; the candle math is chain-agnostic.
...
- **Run ClickHouse in `hybrid` for live terminals.** It backfills history then hands off to a seamless, reorg-safe realtime tail at the chain head. Postgres/MySQL are first-class for historical backfill; the trade templates expand many rows per block, so live/reorg-safe ingestion is ClickHouse-only.
- **Pair with other bundles.** Add [Swaps by Wallet](/data-feeds/templates/markets/swaps-by-wallet) for per-user trade history, or see [Token Analytics](/data-feeds/use-cases/token-analytics) and [Portfolio Tracking](/data-feeds/use-cases/portfolio-tracking) for adjacent template bundles.
