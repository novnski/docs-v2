...
description: "A Data Feeds recipe bundle for compliance, AML, and forensics teams — a complete, ordered, auditable trail of everything an address did, plus its asset positions and risk surface, synced into your own database."
---
...
This use case combines a small set of recipes. Start with **Wallet History** as the chronological backbone, layer **Token Transfers** and **Swaps by Wallet** for asset-level and DEX-activity trails, **Native Balances** for the address's native position, and **Token Approvals** for the standing risk surface.
...
- **Native Balances** and **Token Approvals** describe *state*: the address's native position over time, and the standing allowances it has granted. Approvals are the risk surface — an unrevoked unlimited allowance is live exposure regardless of activity.
...
- **Chains.** Wallet History, Token Transfers, Native Balances, and Swaps by Wallet are chain-parametrized across EVM and Solana; Token Approvals is EVM only.
