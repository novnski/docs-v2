...
description: "A Data Feeds template bundle for portfolio apps, wallets, and dashboards: every token, native asset, and NFT a wallet holds, valued in USD, synced into your own database."
---
...
This use case combines three balance templates, one per asset class, with **Token Prices** for valuation. Each balance template is keyed by wallet, so "everything wallet Y holds" is a single prefix scan.

| Template | Role in the portfolio |
| --- | --- |
| [Token Balances by Wallet](/data-feeds/templates/wallet/token-balances-by-wallet) | Every ERC-20 / SPL token a wallet holds, with its current balance: the core fungible-holdings table, keyed by wallet. |
| [Native Balances](/data-feeds/templates/wallet/native-balances) | The wallet's native-asset (gas-token) balance (ETH, BNB, SOL, etc.), tracked separately from tokens. |
| [NFTs by Wallet](/data-feeds/templates/wallet/nfts-by-wallet) | Every NFT a wallet holds: collection address, token ID, and ERC-721 / ERC-1155 type. |
| [Token Prices](/data-feeds/templates/token/token-prices) | The valuation join target: the latest USD/native mark per token used to value fungible holdings. |

<Note>
  Balances are tracked as **post-balance observations**: each template records the holder's balance *after* every transfer it was part of, so the latest observation per `(wallet, asset)` is the current balance. No periodic re-scan of the chain is needed.
</Note>
...
- **The three balance templates** each unpivot every transfer into per-wallet `(from, post-balance)` and `(to, post-balance)` observations. The latest observation per `(wallet, asset)`, `argMax` over `(block_number, log_index)`, is the live holding. Token and NFT balances are keyed by `(chain_id, wallet_address, token_address[, token_id])`; native balances drop `token_address` and tiebreak on `native_seq` (native transfers carry no `log_index`).
- **Token Prices** is the connective tissue: join each fungible balance to its token's latest mark to value the holding. The template ships a carry-forward `latest_token_price_dict`, so even quiet tokens keep their last on-chain mark: a single `dictGet` per token.
...
- **Native vs token.** The native asset is not an ERC-20 and is tracked by its own template. Don't expect it in the token-balances table. Value it against a native-price mark, not the token-price dictionary.
- **Pair with other templates.** Add [Wallet History](/data-feeds/templates/wallet/wallet-history) for the chronological feed behind a holding, and [Token Approvals](/data-feeds/templates/wallet/token-approvals) to surface outstanding allowances in a wallet's security view.
