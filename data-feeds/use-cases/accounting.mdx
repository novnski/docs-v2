...
description: "A Data Feeds recipe bundle for accounting, tax, and treasury tools — a wallet's full event feed plus per-asset transfer ledgers, valued in USD at block time, synced into your own database."
---
...
| [Token Prices](/data-feeds/recipes/token/token-prices) | The valuation join target — USD/native marks used to value every transfer at block time. |
...
Wallet History   ──┐
Token Transfers  ──┤
Native Transfers ──┼──►  your ledger tables  ◄── valued by ──  Token Prices
NFT Transfers    ──┤
Contract Logs    ──┘   (custom income/expense events — decode against the ABI)
```
...
- **Token Prices** is the connective tissue: join any transfer to the token's mark at its `block_number` to compute the USD value at the time of the event — the basis every tax and accounting calculation needs.
