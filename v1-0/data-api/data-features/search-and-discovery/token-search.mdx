...
- `isVerifiedContract`\
  When `true`, only verified tokens are returned.\
  Default: `false`
- `boostVerifiedContracts`\
  When `true`, verified tokens are ranked higher.\
  Default: `true`
...
Token Search applies additional weighting to **verified tokens** to improve result quality.
...
Token Search is a **premium feature** and requires a specific add-on to be enabled on your plan.
