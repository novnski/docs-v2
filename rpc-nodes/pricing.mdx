...
All [Moralis plans](https://moralis.com/pricing) include generous monthly limits on the number of requests you can make. Some requests are more computationally expensive than others, so heavier requests are assigned higher **Compute Unit (CU) weights**.
...
| Plan       | Price   | CUs Included   | API Throughput | Node Throughput | RPC Nodes |
|------------|---------|----------------|----------------|-----------------|-----------|
| Starter    | $149/mo | 2,000,000/mo   | 40 reqs/s      | 100 CU/s        | 5         |
| Pro        | $249/mo | 100,000,000/mo | 80 reqs/s      | 100 CU/s        | 20        |
| Business   | $749/mo | 500,000,000/mo | 200 reqs/s     | 300 CU/s        | Unlimited |
| Enterprise | Custom  | Custom         | Custom         | Unlimited       | Unlimited |

All plans include full archive data. Prices shown are for monthly billing. Annual billing is also available - see the [pricing page](https://moralis.com/pricing) for current rates.

**API Throughput** is shared across all Moralis products and is measured in requests per second, evaluated over a rolling 4-second window. See [Rate Limits](/data-api/resources/rate-limits) for details.

**Node Throughput** is a separate per-node cap, measured in Compute Units per second on each individual RPC node.
...
If your use case requires more CUs than your current plan offers, consider upgrading or reaching out to the Moralis team for custom solutions at [moralis.com/pricing](https://moralis.com/pricing).
